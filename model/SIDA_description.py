...
class SIDAForCausalLM(LlavaLlamaForCausalLM):
    def __init__(self, config, **kwargs):
        ...
        self.monitor_freq = 5  # Monitor every 100 iterations
    ...
    def model_forward(
        ...
        cls_labels: torch.LongTensor,  # Classification labels: 0-real, 1-full synthetic, 2-part synthetic
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            # Initialize pred_masks as empty list
            pred_masks = []

            # Check if there are any [CLS] tokens and predict class
            if cls_result.size(0) > 0:
                # Use the last [CLS] token for prediction
